using MaceTech.API.Analytics.Domain.Model.Aggregates;
            ...
            public class PotRecordRepository(AppDbContext context) : IPotRecordRepository
            {
                ...
                public async Task<PotRecord?> FindLatestByDeviceIdAsync(long deviceId)
                {
                ...
                public async Task<IEnumerable<PotRecord>> GetRecordsByDeviceIdAndDateRangeAsync(long deviceId, DateTime fromDate, DateTime toDate)
                {
                    return await context.Set<PotRecord>()
