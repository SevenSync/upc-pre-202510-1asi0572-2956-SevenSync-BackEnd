    ...
    ) : ControllerBase
{
    ...
    public async Task<IActionResult> UserLogin([FromBody] GetTokenQuery request)
    {
        ...
        var result = await authenticationQueryService.Handle(request);
    ...
    public async Task<IActionResult> ObtainRefreshToken([FromBody] RefreshTokenCommand command)
    {
        ...
        var refreshTokenModel = mapper.Map<RefreshTokenCommand, CreateRefreshTokenCommand>(command);
        refreshTokenModel.UserId = int.Parse(userId);
        var authorizationResponse = await authenticationCommandService.Handle(refreshTokenModel);
